...
class ItemQtySpec extends ObjectBehavior
{
    ...
    public function it_forces_a_qty_ordered_to_be_supplied()
    {
        $this->beConstructedThrough('fromNative');
        $this->shouldThrow(BadMethodCallException::class)->duringInstantiation();
    }

    public function it_allows_us_to_provide_just_a_qty_ordered()
    {
        $this->beConstructedThrough('fromNative', [1]);
        $this->getOrdered()->toInteger()->toNative()->shouldBe(1);
    }

    public function it_allows_to_get_qty_fulfilled()
    {
        $this->beConstructedThrough('fromNative', [1, 0]);
        $this->getFulfilled()->toInteger()->toNative()->shouldBe(0);
    }

    public function it_returns_zero_qty_fulfilled_if_no_argument_was_provided()
    {
        $this->beConstructedThrough('fromNative', [1]);
        $this->getFulfilled()->toInteger()->toNative()->shouldBe(0);
    }
    ...
    public function it_wont_allow_us_to_fulfill_more_than_we_order()
    {
        $this->beConstructedThrough('fromNative', [1, 2]);
        $this->shouldThrow(InvalidArgumentException::class)->duringInstantiation();
    }

    public function it_wont_allow_us_to_fulfill_more_than_we_order_for_negative_qty()
    {
        $this->beConstructedThrough('fromNative', [-1, -2]);
        $this->shouldThrow(InvalidArgumentException::class)->duringInstantiation();
    }

    public function it_wont_allow_us_to_fulfill_more_than_we_order_for_incompatible_qty()
    {
        $this->beConstructedThrough('fromNative', [-1, 2]);
        $this->shouldThrow(InvalidArgumentException::class)->duringInstantiation();
    }
